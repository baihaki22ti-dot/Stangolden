<template>
  <div class="min-h-screen bg-gray-100">
    ...
    <div :class="['flex-1 flex flex-col transition-all duration-300', isOpen ? 'md:ml-64' : 'md:ml-0']">
      <Navbar
        title="UPKP"
        description="Pilih submodul UPKP"
      />
      <main class="p-6 max-w-7xl w-full mx-auto space-y-6 flex-1">
        <div class="flex justify-center">
          <div class="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-6 w-max">
            <router-link v-if=isAdminUser
              v-for="child in category.children"
              :key="`admin-${child.slug}`"
              :to="`/moduladm/upkp/${child.slug}`"
...
const category = findCategory('upkp')
const image = ref('')
const me = ref(null) 
