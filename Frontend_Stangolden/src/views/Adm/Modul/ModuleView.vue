          ...
          >
            <div class="bg-white rounded-xl shadow-xl max-w-sm w-full p-5">
              ...
              <p class="text-sm text-slate-600">
                Silahkan hubungi admin untuk mengakses fitur ini.
              </p>
              <div class="mt-4 flex justify-end gap-2">
                <button class="px-3 py-1.5 text-sm rounded bg-slate-100" @click="lockPopupOpen=false">Tutup</button>
                <!-- <button class="px-3 py-1.5 text-sm rounded bg-sky-600 text-white" @click="contactAdmin">Hubungi Admin</button> -->
              </div>
            </div>
          </div>
        </transition>
...
function contactAdmin() {
  lockPopupOpen.value = false
  alert('Silahkan hubungi admin melalui saluran resmi.')
}
...
const defaultSub = {
  upkp: 'wawasan-kebangsaan',
  'tugas-belajar': 'panduan-tugas-belajar'
}

// Bangun tujuan navigasi untuk modul yang terbuka
function buildTo(m) {
  const group = m.slug
  if (isAdminUser.value) {
    if (group === 'upkp' && hasRoute('ModulUPKPadmin')) return { name: 'ModulUPKPadmin' }
    if (group === 'tugas-belajar' && hasRoute('ModulTugasBelajaradmin')) return { name: 'ModulTugasBelajaradmin' }
    return { name: 'ModulDetailAdmin', params: { group, slug: defaultSub[group] } }
  } else {
    if (group === 'upkp' && hasRoute('ModulUPKPsiswa')) return { name: 'ModulUPKPsiswa' }
    if (group === 'tugas-belajar' && hasRoute('ModulTugasBelajarsiswa')) return { name: 'ModulTugasBelajarsiswa' }
    return { name: 'ModulDetailSiswa', params: { group, slug: defaultSub[group] } }
  }
}
